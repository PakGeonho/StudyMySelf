...
public class CrytoGenerator {
	...
	// 반환값 : Map<String, String> 공개키의 가수부, 지수부 분리해서 저장 후 클라이언트에 제공
	// 파라미터 : HttpSession - 비밀키 저장 활용.
	public static Map<String, String> generatePairKey(HttpSession session){//return하는 Map이 바로 공개키이다.
		// 공개키, 비밀키 생성
		...
		// 한국 인터넷 진흥원 : RSA + SHA256 병행 사용(권장)
		try {
			keyGenerator = KeyPairGenerator.getInstance("RSA");
			// RSA 암호화 알고리즘을 통해서 암호화 결과값의 사이즈 -> 항상 짝수여야함.
			keyGenerator.initialize(2048);
			// 공개키와 비밀키를 생성
			keyPair = keyGenerator.generateKeyPair(); //keyPair가 생성됨. => 공개키와 비밀키 모두 갖는다.
			//공개키 취득
			publicKey = keyPair.getPublic();
			//비밀키 취득
			privateKey = keyPair.getPrivate(); // 이 비밀키를 HttpSession에 저장함.
			session.setAttribute("privateKey", privateKey);
			
			// 공개키(Double타입)를 가수부, 지수부 분리해서 반환
			...
			RSAPublicKeySpec keySpec = (RSAPublicKeySpec)keyFactory.getKeySpec(publicKey, RSAPublicKeySpec.class);//RSAPublicKeySpec.class => .class를 붙여 타입을 선언해준 모습.
