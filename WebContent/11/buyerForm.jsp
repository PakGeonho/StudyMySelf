...
<script type="text/javascript" src='/ddit/js/buyerValidation.js'></script>
<script type="text/javascript">
	$(function(){
		...
		$('#search_buyer_id').click(function(){
			var url = '<%= request.getContextPath()%>/11/lprodForBuyer.jsp';
			var options = 'copyhistory=no,width=380px,height=400px,status=no,resizable=no,scrollbars=no';
			window.open(url, '거래처 코드 검색', options);
		});
		
		
		$('#btn3').click(function(){
			location.href = "<%=request.getContextPath()%>/11/main.jsp?contentPage=buyerList.jsp";
		});
	})	                                      
</script>
<body>
<form name="buyerForm" method="post" action="<%=request.getContextPath() %>/11/insertBuyerInfo.jsp">
